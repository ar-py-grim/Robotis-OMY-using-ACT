...
class SimpleEnv:
    ...
    def grab_image(self):
        ...
        # self.rgb_top = self.env.get_fixed_cam_rgbd_pcd(cam_name='topview')
    ...
    def get_joint_state(self):
        ...
        gripper_cmd = 1.0 if self.gripper_state else 0.0
    ...
    def teleop_robot(self):
        ...
        returns:
            ...
            done: bool, True if the user wants to end this episode
        ...
        if self.env.is_key_pressed_once(key=glfw.KEY_Z):
            return np.zeros(7, dtype=np.float32), True, False   
        
        if self.env.is_key_pressed_once(key=glfw.KEY_SPACE):
            self.gripper_state =  not self.gripper_state


        if self.env.is_key_pressed_once(key=glfw.KEY_X):
            return np.zeros(7, dtype=np.float32), False, True
