...
from .transforms import (t2p, t2r, pr2t,
    r2quat, quat2r, r2w, rpy2r,
    meters2xyz, get_rotation_matrix_from_two_points,
)
from .utils import (
    trim_scale, compute_view_params,
    get_idxs, get_colors,
    get_monitor_size, TicTocClass,
)
...
class MinimalCallbacks:
    ...
    def _key_callback(self, window, key, scancode, action, mods):
        ...
        # Pause / resume handling (p)
        if is_key_pressed and (key==glfw.KEY_P) and (self._paused is not None):
    ...
    def _cursor_pos_callback(self, window, xpos, ypos):
        ...
        with self._gui_lock:
            if self.pert.active:
                mujoco.mjv_movePerturb(self.model, self.data, action,
                    dx/height, dy/height, self.scn, self.pert)
            else:
                mujoco.mjv_moveCamera(self.model, action,
                    dx/height, dy/height, self.scn, self.cam)
    ...
    def _scroll_callback(self, window, x_offset, y_offset):
        with self._gui_lock:
            mujoco.mjv_moveCamera(self.model, mujoco.mjtMouse.mjMOUSE_ZOOM, 0, -0.05*y_offset,
                                 self.scn, self.cam)
...
class MuJoCoMinimalViewer(MinimalCallbacks):
    ...
    def _add_marker_to_scene(self, marker):
        ...
        for key, value in marker.items():
            ...
            if isinstance(value, (int, float, mujoco._enums.mjtGeom)):
                ...
                raise ValueError("mjtGeom has attr {} but type {} is invalid".format(
                        key, type(value)))
    ...
    def add_rgb_overlay(self,rgb_img_raw,fix_ratio=False):
        ...
        if fix_ratio: # fix aspect ratio
            ...
            # Final resized and padded image
            rgb_img_rsz = padded_img 
...
class MuJoCoParserClass(object):
    ...
    def _parse_xml(self):
        ...
        # Degree of Freedom
        self.n_dof = self.model.nv # (dof = # of columns of Jacobian)
    ...
    def store_state(self):
        ...
        self.state_stored = copy.deepcopy(state)
    ...
    def set_T_body(self,body_name='base',p=np.array([0,0,0]),R=np.eye(3),T=None,forward=True):
        ...
        Parameters:
            ...
            p (np.array): New position.
            R (np.array): New rotation matrix.
            T (np.array): Transformation matrix that overrides p and R if provided.
            forward (bool): Whether to perform forward kinematics after updating.
    ...
    def get_pcd_from_depth_img(self,depth_img,fovy=45):
        ...
        cam_matrix = np.array(((focal_scaling,0,img_width/2),
                            (0,focal_scaling,img_height/2), (0,0,1)))
    ...
    def get_fixed_cam_rgb(self,cam_name):
        ...
        # Update
        mujoco.mjv_updateScene(self.model, self.data, self.viewer.vopt, self.viewer.pert,
    ...
    def plot_time(self, loc = 'bottom left'):
        """
    ...
    def plot_sensors(self, loc = 'bottom right'):
        """
